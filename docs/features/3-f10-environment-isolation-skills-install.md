...
(2) **teardown 链**：Orchestrator 在 run 终结（成功 / 失败 / 取消）统一调 `EnvironmentIsolator.teardown_run(run_id, paths)` → `HomeMtimeGuard.diff_against(before_snapshot)` → `WorkdirScopeGuard.assert_scope(workdir, before=setup_before_set)` → 合并两份 report 写入 audit `env.teardown`（F02 `AuditWriter.append`）；
...
| `harness.persistence.AuditWriter.append` (IAPI-009) | `harness/persistence/audit.py`（F02 导出） | F10 `env.setup` / `env.teardown` / `skills.install` 事件应写入 audit JSONL；直接复用 F02 append-only writer，避免再造一份写盘逻辑。 |
