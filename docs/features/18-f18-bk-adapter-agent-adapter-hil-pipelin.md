...
**Wave 4.1 unified Esc-text 协议（2026-04-27）**：在 Wave 4 hook bridge 闭环 PoC 通过后，主 agent 通过实测 3 种 reply 方案对 PreToolUse(AskUserQuestion) 的语义反应（详见穿刺报告 reference/f18-tui-bridge/evidence-summary.md）：

| 方案 | 应答键序 | hook 序列 | 结论 |
|---|---|---|---|
| A (默认) | `ESC + bracketed-paste(merged_text) + CR` | SessionStart → UserPromptSubmit → PreToolUse(AskUserQuestion) → **UserPromptSubmit** → **Stop** | claude DONE 输出 + 不重问；audit 闭环走 PreToolUse + 第 2 个 UserPromptSubmit + Stop |
| C (baseline) | `<N>\r` 单选 | SessionStart → UserPromptSubmit → PreToolUse(AskUserQuestion) → PostToolUse(AskUserQuestion) → Stop | 仅菜单选项；TUI 改菜单形态会破坏协议 |

裁决：**A 作为默认协议**——单一通道把 single-select / multi-select / multi-question / freeform 四类 HIL 答案合并为一段文本一次注入，与 TUI 解耦，不依赖菜单形态。Baseline `<N>\r` 路径降级为 `HilWriteback(prefer_baseline=True)` 兼容 fallback。settings.json hook 注册集合从 4 类扩展为 8 类（新增 `Stop / SubagentStop / UserPromptSubmit / Notification` 四类 catch-all）支持新 audit 闭环。
...
- **Key types（Wave 4 后）**:
  ...
  - **[NEW / Wave 4.1 MOD]** `harness.hil.TuiKeyEncoder` (`harness/hil/tui_keys.py`) — `HilAnswer` → TUI 键序 bytes。**Wave 4.1**：新增 `encode_unified_answer(text) → b"\x1b" + b"\x1b[200~" + text.encode("utf-8") + b"\x1b[201~" + b"\r"`，作为 HIL 答案 default 编码（覆盖 single/multi/multi-question/freeform 一次注入）；保留 `encode_radio / encode_checkbox / encode_freeform / encode_interrupt / encode_exit` 作 baseline fallback；`_validate_freeform_text` helper 提取为模块级函数复用。
  - **[NEW / Wave 4.1 MOD]** `harness.orchestrator.HookEventToStreamMapper` (`harness/orchestrator/hook_to_stream.py`) — hook event → `TicketStreamEvent` envelope（wire 层别名 `StreamEvent`）。**Wave 4.1**：`TicketStreamKind` Literal 扩展 4 个 audit kind（`turn_complete / user_prompt_submit / subagent_complete / notification`）；`map` 处理 4 个新 hook 类型（Stop / UserPromptSubmit / SubagentStop / Notification）。
  ...
  - **[MOD / Wave 4.1 MOD]** `harness.hil.HilWriteback` — payload 由 JSON 改为 TUI 键序 bytes，落地经 IAPI-021。**Wave 4.1**：默认走 unified Esc-text（`_merge_text(question, answer)` → `encode_unified_answer(merged_text)`）；构造函数新增 `prefer_baseline: bool = False`；新增对 `HilEventBus.publish_answered_via_prompt(merged_text=...)` 的调用；`_encode_baseline` 保留作兼容路径；audit 闭环走 PreToolUse + UserPromptSubmit + Stop（PostToolUse 不 fire — 设计预期）。
  - **[MOD / Wave 4.1 MOD]** `harness.hil.HilEventBus` — Wave 4 含 `publish_opened / publish_answered / tool_use_id_queue`；**Wave 4.1** 新增 `publish_answered_via_prompt(*, ticket_id, run_id, merged_text)`，写 audit `hil_answered` 事件含 `payload.answer.value=merged_text` + `channel="unified_esc_text"`，与 baseline `publish_answered(answer=HilAnswer)` 并列存在。
  - **[MOD / Wave 4.1 MOD]** `harness.adapter.workdir_artifacts.SettingsArtifactWriter.write` — Wave 4.1 settings.json `hooks` 字段从 4 类扩展为 8 类，新增 `Stop / SubagentStop / UserPromptSubmit / Notification` 四类 catch-all entry。
  - **[MOD / Wave 4.1 MOD]** `harness.adapter.HookEventPayload` — Wave 4.1 `HookEventName` Literal 扩展为 8 类。
  - **[MOD / Wave 4.1 MOD]** `harness.cli_dialog.models.ActionKind` — 新增 `"unified_answer"`；`harness.cli_dialog.actuator.DialogActuator.encode` 处理 `unified_answer` 分支（`ESC + bracketed-paste(text) + CR`）；`harness.cli_dialog.catalog.DEFAULT_POLICIES` 新增 `"hil-ask-user-question"` 默认 action 占位（驱动器运行时构造合并文本）。
  - **[MOD / Wave 4.1 MOD]** `harness.orchestrator.run.run_real_hil_round_trip` — 答案阶段从 `encode_radio(1)` 改为 `encode_unified_answer(merged_text)`；`_RealHilRoundTripResult.hook_fires` 语义改为 PreToolUse + UserPromptSubmit + Stop 联合计数；优先走 `bus.publish_answered_via_prompt`。
...
| `TuiKeyEncoder.encode_unified_answer` **[Wave 4.1 NEW]** | `encode_unified_answer(text: str) -> bytes` | `text` 不含禁用控制字符 `\x03 / \x04 / \x1b`（协议前缀 ESC + paste-start/end 由本方法添加） | 返 `b"\x1b" + b"\x1b[200~" + text.encode("utf-8") + b"\x1b[201~" + b"\r"`；覆盖 single-select / multi-select / multi-question / freeform 四类合并文本一次注入；UTF-8 多字节守恒；空字符串合法（仅协议前缀 + 空 paste body + CR）；`text` 是 harness UI 端把所有 question 答复合并的最终文本 | `EscapeError`（text 含 `\x03 / \x04 / \x1b`） |
| `TuiKeyEncoder.encode_interrupt` | `encode_interrupt() -> bytes` | — | 返 `b"\x03"`（中断当前 turn） | 不抛 |
| `HookEventToStreamMapper.map` **[Wave 4.1 MOD]** | `map(payload: HookEventPayload, ticket_id: str, seq: int) -> TicketStreamEvent` | seq 单调递增 | 返 `TicketStreamEvent(ticket_id, seq, ts=str(payload.ts), kind, payload=dict)`；`kind` 派生矩阵（Wave 4 + 4.1 共 8 类）：`SessionStart/SessionEnd → "system"`；`PreToolUse + tool_name in {AskUserQuestion,Question} → "tool_use"`；`PreToolUse + 其他 tool → "tool_use"`；`PostToolUse → "tool_result"`；**Wave 4.1**：`Stop → "turn_complete"` / `UserPromptSubmit → "user_prompt_submit"` / `SubagentStop → "subagent_complete"` / `Notification → "notification"`；`payload` dict 含原 hook payload 字段（不带 type） | 不抛 |
...
| `HilWriteback.write_answer` **[Wave 4.1 MOD]** | `write_answer(*, ticket_id: str, question: HilQuestion, answer: HilAnswer) -> None` | ticket state == `hil_waiting`；`answer` 已通过 `HilControl` 校验 | **default · unified Esc-text**：(1) `_merge_text(question, answer)` 把 selected_labels (", ".join) + freeform_text (优先) 合并为 `merged_text`；(2) `TuiKeyEncoder.encode_unified_answer(merged_text)` → bytes；(3) base64 + POST `/api/pty/write`（IAPI-021）；(4) 成功 → `HilEventBus.publish_answered_via_prompt(ticket_id, run_id, merged_text)`（audit `hil_answered` + payload 含 `answer.value=merged_text` / `channel="unified_esc_text"` + WS broadcast）+ ticket `hil_waiting → classifying`；(5) 失败（pty 已关闭）→ 答案保留至 `pending_answers` + ticket `hil_waiting → failed`（FR-011 AC-4）。**baseline · `prefer_baseline=True`**：path (1) 改为 `_encode_baseline` 复用 Wave 4 旧逻辑（radio/checkbox/freeform）；audit 走 `publish_answered(answer=HilAnswer)`。 | `PtyClosedError`；`EscapeError`（merged_text 或 freeform 含禁用控制字符 `\x03 / \x04 / \x1b`）；`HTTPException`（IAPI-021 4xx 透传） |
| `HilEventBus.publish_answered_via_prompt` **[Wave 4.1 NEW]** | `publish_answered_via_prompt(*, ticket_id: str, run_id: str, merged_text: str) -> None` | 调用方为 `HilWriteback`（unified Esc-text 默认路径）；与 `publish_answered(answer=HilAnswer)` 并列存在 | 写 audit `hil_answered` 事件（`payload.answer = {value: merged_text, channel: "unified_esc_text"}`）+ WS broadcast 同 payload；不修改 ticket 状态（由调用方负责） | 不抛 |
...
**6. Wave 4.1 unified Esc-text 协议增量（2026-04-27）**：在 Wave 4 PoC 闭环验证后，主 agent 通过实测三方案（Esc-text vs `<N>\r` vs 其他）裁决 unified Esc-text 为 HIL 默认应答协议。修改清单：
- `harness/adapter/hook_payload.py`：`HookEventName` Literal 8 类
- `harness/adapter/workdir_artifacts.py::SettingsArtifactWriter.write`：settings.json 注册 8 类 hooks（前 4 类原 matcher，后 4 类 catch-all）
- `harness/orchestrator/hook_to_stream.py::HookEventToStreamMapper`：`TicketStreamKind` 加 4 个 audit kind；`map` 处理 4 个新 hook 类型
- `harness/hil/event_bus.py::HilEventBus`：新增 `publish_answered_via_prompt(*, ticket_id, run_id, merged_text)`
- `harness/hil/tui_keys.py::TuiKeyEncoder`：新增 `encode_unified_answer(text)`；`_validate_freeform_text` 提取为模块级 helper
- `harness/hil/writeback.py::HilWriteback`：默认走 unified 路径（`_merge_text` + `encode_unified_answer` + `publish_answered_via_prompt`）；构造函数 `prefer_baseline=False` 控制 fallback；`_encode_baseline` 保留 Wave 4 旧逻辑
- `harness/cli_dialog/models.py::ActionKind`：新增 `"unified_answer"`
- `harness/cli_dialog/actuator.py::DialogActuator.encode`：处理 unified_answer 分支
- `harness/cli_dialog/catalog.py::DEFAULT_POLICIES`：新增 `"hil-ask-user-question"` 占位 policy
- `harness/orchestrator/run.py::run_real_hil_round_trip`：答案阶段切到 unified；`_RealHilRoundTripResult.hook_fires` 语义改为 PreToolUse + UserPromptSubmit + Stop 联合计数（broadcaster.events 全量计数自然实现）

调用链修订：`Frontend → HilAnswerSubmit(merged_text) → POST /api/hil/:ticket_id/answer → HilWriteback.write_answer → _merge_text → TuiKeyEncoder.encode_unified_answer → POST /api/pty/write → PtyWorker.write → claude TUI stdin (ESC + paste(merged_text) + CR) → claude TUI 渲染 "● User answered..." 单行；hook bridge 派出 PreToolUse(AskUserQuestion) + UserPromptSubmit + Stop 三 event` → audit 链 `hil_captured` (PreToolUse) → `hil_answered` (UserPromptSubmit / 由 `publish_answered_via_prompt` 写入) → `turn_complete` (Stop)。

**Wave 4.1 设计预期**：unified Esc-text 路径下 PostToolUse(AskUserQuestion) 不 fire（claude TUI 把 unified-paste 当作 user prompt 而非 tool answer，跳过 PostToolUse 路径直接走 UserPromptSubmit + Stop）。这是 claude CLI v2.1.119 的实测行为，不是 bug；baseline `<N>\r` 路径下 PostToolUse 仍然 fire（兼容回归）。
...
| **T-UNIFIED-RADIO** | **FUNC/happy** | **FR-053 Wave 4.1 AC-default · 单选合并文本一次注入** | `TuiKeyEncoder().encode_unified_answer("Python")` | bytes `== b"\x1b\x1b[200~Python\x1b[201~\r"`（byte-equal） | unified Esc-text 协议前缀字节错位 |
| **T-UNIFIED-MULTI-SELECT** | **FUNC/happy** | **FR-053 Wave 4.1 AC-default-multi** | `encode_unified_answer("Python, Go")`（multi-select labels 经 `_merge_text` 用 `", ".join()` 合并）| bytes `== b"\x1b\x1b[200~Python, Go\x1b[201~\r"` | 多选合并语义错（顺序错 / 分隔符错） |
| **T-UNIFIED-MULTI-QUESTION** | **FUNC/happy** | **FR-053 Wave 4.1 AC-default · 多 question 合并** | `encode_unified_answer("Lang: Python; Test: pytest; CI: github-actions")` | bytes 等于协议前缀 + 合并文本 + 后缀 + CR；UTF-8 守恒 | 多 question 串行回写残余（旧逻辑）/ 顺序错 |
| **T-UNIFIED-FREEFORM** | **FUNC/happy** | **FR-053 Wave 4.1 AC-default-utf8 · freeform 合并** | `encode_unified_answer("我想要 Rust 语言 — 因为速度")` | bytes byte-equal（UTF-8 多字节守恒） | UTF-8 被破坏 / freeform 走错协议 |
| **T-UNIFIED-SEC** | **SEC/inject** | **FR-053 Wave 4.1 AC-default-sec** | `encode_unified_answer("safe\x03injected")` / `encode_unified_answer("color\x1b[31mred")` | `EscapeError`（控制字符 `\x03 / \x04 / \x1b` 全部拒绝） | 协议外 ESC 注入未拦 |
| **T-MULTI-ROUND** | **INTG/cli (mark `real_cli`)** | **FR-013 PoC + FR-053 Wave 4.1 unified path** | 跨 user_turn 3 轮 HIL × unified Esc-text；每轮 `merged_text` 不同 | 每轮 `posted` 列表新增一段 unified bytes；audit `hil_answered` × 3，每条 `payload.answer.value == merged_text`；ticket 状态每轮 `hil_waiting → classifying` | 跨轮 audit 错乱 / 合并逻辑跨轮残留 |
| **T-STOP-AUDIT** | **FUNC/happy** | **Wave 4.1 audit chain Stop hook** | `HookEventToStreamMapper.map` 处理 `hook_event_name="Stop"` payload | `TicketStreamEvent.kind == "turn_complete"`；`payload.hook_event_name == "Stop"` | Stop hook 误派 system / 漏 enum case |
| **T-USER-PROMPT-SUBMIT-AUDIT** | **FUNC/happy** | **Wave 4.1 audit chain UserPromptSubmit hook + publish_answered_via_prompt** | (a) `mapper.map` 处理 `UserPromptSubmit` → kind `"user_prompt_submit"`；(b) `bus.publish_answered_via_prompt(merged_text="Python, Go")` → audit `hil_answered` 事件 `payload.answer == {"value": "Python, Go", "channel": "unified_esc_text"}` + WS broadcast 同 payload | 两支断言全过 | UserPromptSubmit 错派 / merged_text 丢失 / channel 标识缺失 |
| **T-BASELINE-COMPAT** | **FUNC/happy** | **Wave 4 兼容回归 · `prefer_baseline=True`** | `HilWriteback(..., prefer_baseline=True).write_answer(...)` 走 baseline 路径 | `posted == [b"1\r"]`（旧 `<N>\r` 编码）；audit 走 `publish_answered`（不是 `publish_answered_via_prompt`） | 兼容路径破坏 / 默认与 fallback 互相污染 |

**Test Inventory 统计**：49 行（T01-T39 + T-HOOK-SCHEMA-CANARY + Wave 4.1 NEW × 9：T-UNIFIED-RADIO/MULTI-SELECT/MULTI-QUESTION/FREEFORM/SEC + T-MULTI-ROUND + T-STOP-AUDIT + T-USER-PROMPT-SUBMIT-AUDIT + T-BASELINE-COMPAT）。负向 / 边界 / 安全行（FUNC/error + BNDRY/* + SEC/* + UT/SEC + UT/BNDRY canary）：T03/T04/T05/T08/T09/T10/T12/T14/T16/T17/T19/T20/T22/T24/T26/T28/T31/T36/T38/T39 + T-HOOK-SCHEMA-CANARY = **21 行 / 40 行 = 52.5%**（≥ 40% 阈值通过）。
