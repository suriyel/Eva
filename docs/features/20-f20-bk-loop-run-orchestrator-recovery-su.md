...
| `reenqueue_ticket` | `async reenqueue_ticket(self, cmd: TicketCommand) -> None` | 由 `AnomalyRecovery.decide → RecoveryDecision(kind="retry")` 触发；`cmd.kind="retry"` 且 `skill_hint` 与原 ticket 相同 | 在 `RetryPolicy.next_delay` 返回的延迟后调用 `run_ticket(cmd)`；新 ticket 的 `parent_ticket` 指向被恢复的 ticket；`retry_count` 由 `RetryCounter.inc(skill_hint, cls)` 递增 | `MaxRetriesExceeded`（`RetryPolicy.next_delay` 返 `None` 时）|
| `cancel_ticket` | `async cancel_ticket(self, ticket_id: str) -> None` | ticket 对应进程仍存活 | 向 pty 发 SIGTERM；5s 后未终止则 SIGKILL；ticket 转 `aborted`；audit `cancel_ticket` | `TicketNotFound` |
