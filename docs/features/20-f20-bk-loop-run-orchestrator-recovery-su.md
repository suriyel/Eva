...
| `RunOrchestrator.on_signal` | `async on_signal(event: SignalEvent) -> None` | event.kind ∈ SignalEvent Literal 集 | bus 已 broadcast；`bugfix_request` / `increment_request` 时主循环下一迭代必经 phase_route 重路由（FR-003 / FR-048） | — |
...
| `TicketSupervisor.reenqueue_ticket` | `async reenqueue_ticket(cmd: TicketCommand) -> None` | RetryPolicy.next_delay 返回非 None；retry_count < 阈值 | RetryCounter.inc；按 delay 等待后将 cmd 推回主循环（FR-024/025/026） | — |
| `TicketSupervisor.cancel_ticket` | `async cancel_ticket(ticket_id: str) -> None` | ticket 在跑 | 当前 pty 进程 SIGTERM；watchdog disarm；ticket state→"aborted" | — |
...
| `TicketSupervisor.reenqueue_ticket` | T22/T23/T25 间接（通过 RetryPolicy 回路） |
| `TicketSupervisor.cancel_ticket` | T15 间接 |
