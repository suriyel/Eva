...
|---|---|---|---|---|---|
| **INT-001** | **HIL full round-trip（hooks + TUI 键序）[Wave 4 REWRITE / Wave 4.1 unified Esc-text 默认]**：spawn claude → 1× PreToolUse(AskUserQuestion) hook fire → harness HookEventMapper → HilQuestion → harness UI 渲染控件 → 用户合并答案为 merged_text → TuiKeyEncoder.encode_unified_answer → POST /api/pty/write → claude TUI 续跑 → 第 2 个 UserPromptSubmit + Stop hook 关闭 audit 闭环 | F18, F21, F20, F02 | claude pty 子进程触发 PreToolUse(AskUserQuestion) hook → hook 子进程 stdin JSON envelope → POST /api/hook/event (IAPI-020) → HookEventMapper → HilQuestion → HilEventBus → IAPI-001 `/ws/hil` push → UI → POST /api/hil/:ticket_id/answer (IAPI-002 [Wave 4 MOD]) → HilWriteback.write_answer (IAPI-007 [Wave 4 MOD / Wave 4.1 MOD]) → `_merge_text(question, answer)` → TuiKeyEncoder.encode_unified_answer (Wave 4.1 default `ESC + paste(merged_text) + CR`) → POST /api/pty/write (IAPI-021) → PtyWorker.write → claude TUI 续跑（unified 路径下无 PostToolUse fire — 设计预期） → 第 2 个 UserPromptSubmit + Stop hook → IAPI-020 二次回流 → audit 闭环 PreToolUse + UserPromptSubmit + Stop 三 hook 联合判定 → IAPI-009 AuditWriter | 整链 p95 < 3s；同 pid 续跑；ticket.hil.answers 持久化；FR-013 PoC gate (≥95% 成功率 / 20 轮跨 user_turn HIL × hook + unified Esc-text 键序回写真测试)；**Wave 4.1 invariants 修订**：T29 hook fire 含 PreToolUse + UserPromptSubmit + Stop（不再依赖 PostToolUse）；audit `hil_answered` payload `value=merged_text` + `channel="unified_esc_text"`；**SEC**: sha256(~/.claude/settings.json) + sha256(~/.claude.json) run 前后字节级守恒（NFR-009 强化 + FR-051 AC）；**SEC**: bracketed paste 合并文本 byte-equal 守恒（FR-053 Wave 4.1 AC）；**新增子场景 T-MULTI-ROUND**：跨 user_turn 3 轮 HIL × unified Esc-text；每轮 merged_text 不同，audit `hil_answered` × 3 each carrying merged_text + channel | System ST |
