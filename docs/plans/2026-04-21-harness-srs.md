...
### 1.4 现有系统接口（Existing System Interface · ESI）

<!-- Wave 4: Added 2026-04-27 — F18 协议层重构 (stream-json → TUI + Hook Bridge) 反向回填 ESI；证据来源 reference/f18-tui-bridge/{README.md, evidence-summary.md, puncture.py} -->

**变更类型分布（Wave 4）**: NEW=3 (FR-051/052/053) · MODIFY=5 (FR-008/009/011/015/016 + IFR-001) · DEPRECATED=1 (FR-014) · ASSUMPTIONS NEW=2 (ASM-009/010)

**ESI 已确立维度** — Harness 在 spawn Claude Code 子进程并执行 HIL 闭环时所依赖的外部系统接口契约（CLI 版本 + 配置文件字段 + 隔离写路径白名单），构成 Harness 与 claude CLI 之间的稳定边界：

1. **claude CLI 版本约束**: ≥ **2.1.119**（hook stdin schema 与 TUI 键序在此版本起实测稳定，证据见 `reference/f18-tui-bridge/evidence-summary.md` v8 实测时间线 + `reference/f18-tui-bridge/puncture.py` 可重跑穿刺）。CLI 升级若改动 hook stdin schema 字段名/嵌套结构（受 ASM-009 约束）或 TUI 菜单交互方式（受 ASM-010 约束），需触发本节 ESI 修订与对应 mapper/encoder 更新。

2. **`settings.json` 字段依赖清单**（workdir-scoped，仅写入 `.harness-workdir/<run-id>/.claude/settings.json`）:
   - `env`: provider routing 注入点（不含 `ANTHROPIC_*` 凭证；继承 `claude auth login`）
   - `hooks.PreToolUse(AskUserQuestion)`: HIL 捕获通道（FR-009）
   - `hooks.PostToolUse(*)`: 工具结果回流（lifecycle 锚定；baseline `<N>\r` 路径下作为答案 audit）
   - `hooks.SessionStart`: ticket 生命周期起点
   - `hooks.SessionEnd`: ticket 生命周期终点（取代 FR-014 banner 检测）
   - **Wave 4.1 (2026-04-27) NEW**: `hooks.Stop / SubagentStop / UserPromptSubmit / Notification` 四类 catch-all hooks，支撑 unified Esc-text 协议（FR-053 默认）的 audit 闭环（PreToolUse + UserPromptSubmit + Stop）。settings.json `hooks` 总字段数从 4 类扩展为 8 类。
   - `enabledPlugins`: longtaskforagent bundle 启用项
   - `model`: per-ticket 模型覆写入口（FR-019/020 协同）
   - `skipDangerousModePermissionPrompt`: TUI 侧权限提示绕过（与 argv `--dangerously-skip-permissions` 协同）

3. **`.claude.json` 字段依赖清单**（workdir-scoped，仅写入 `.harness-workdir/<run-id>/.claude.json`）:
   - `hasCompletedOnboarding`: 强制 `true` 跳过首启 onboarding
   - `projects.<cwd>.hasTrustDialogAccepted`: 强制 `true` 跳过 workdir 信任对话
   - `lastOnboardingVersion`: 与当前 CLI 版本一致以避免 onboarding 重弹
   - `projectOnboardingSeenCount`: ≥ 1 表示已见过项目级 onboarding

4. **隔离写路径白名单**: 仅允许写 `.harness-workdir/<run-id>/.claude*`（即 `.claude.json` 文件 + `.claude/` 目录树）；禁止写入用户级 `~/.claude/` 与 `~/.claude.json`（NFR-009 强化）。

**ESI 复用的横切关注点**:
- **NFR-009**（环境清洁）由本节 ESI 隔离写路径白名单强化为可测断言：run 前后 sha256 比对 `~/.claude/settings.json` + `~/.claude.json` 必须不变（FR-051 AC + IFR-001 新 AC）
- **CON-007**（Harness 不写入 ~/.claude/）通过 ASM-009 的字段稳定性假设落地为具体的 settings.json/`.claude.json` 8+4 字段契约
- **ASM-002**（claude CLI 在 PATH 可调用）通过 ASM-009/010 进一步细化为 v2.1.119 版本约束

**ESI 受影响模块（Wave 4 重构范围）**:
- **Hard impact**: F18 (Bk-Adapter Agent Adapter & HIL Pipeline)、F20 (Bk-Loop Run Orchestrator · Recovery · Subprocess) — 协议层从 stream-json 全面切换为 TUI + Hook Bridge
- **Soft impact**: F3/F10 (Environment Isolation — `prepare_workdir` 三件套预置)、F21 (Fe-RunViews — TicketStream envelope rename + HILInbox 副文案)、F23 (Bugfix REST/WS — 新增 `/api/hook/event` + `/api/pty/write` router include)
- **未受影响模块**: F1 (App Shell)、F2 (Persistence Core)、F12 (Frontend Foundation)、F19 (Bk-Dispatch Model Resolver & Classifier)、F22 (Fe-Config)、F17 (PyInstaller — 仍 failing 等待 F18 解锁)

**变更摘要**: Wave 4 把 F18 的 HIL 协议层从 `--output-format stream-json --include-partial-messages` 全面切换为「workdir-scoped settings.json hooks (PreToolUse(AskUserQuestion)/PostToolUse(*)/SessionStart/SessionEnd) → harness POST /api/hook/event 桥」+「PTY TUI 键序回写（option 序号 `<N>\r` / 自由文本 bracketed paste / 表单串行写键）」。stream-json 解析器、JsonLinesParser、HilExtractor、BannerConflictArbiter 模块整体弃用；FR-014 (banner+HIL 冲突仲裁) 由 SessionEnd hook 必触发后的 event_name + tool_use_id queue 处理替代，故标 [DEPRECATED Wave 4] 但不删除文字以保留可追溯性。
...
### FR-008: 用交互模式 (TUI) + PTY 包装运行 Claude Code，HIL 通过 hook bridge 捕获
**优先级**: Must
<!-- Wave 4: Modified 2026-04-27 — 从 stream-json 协议切换为 TUI + workdir-scoped settings.json hooks bridge；argv 永禁 -p/--print/--output-format/--include-partial-messages -->
**EARS**: The system shall 通过交互模式（TUI）+ PTY 包装运行 Claude Code，argv 永禁 `-p / --print / --output-format / --include-partial-messages`；HIL 捕获改由 workdir-scoped settings.json hooks (PreToolUse(AskUserQuestion)) 通道转发到 harness HTTP `POST /api/hook/event`。
**可视化输出**: N/A — backend-only
**验收准则**:
- Given spawn Claude ticket，when 检查 argv，then 不含 `-p` / `--print` / `--output-format` / `--include-partial-messages`
- Given spawn 后 skill 调 AskUserQuestion，when hook bridge 接收，then PreToolUse(AskUserQuestion) hook fire 计数 == AskUserQuestion 调用数 / harness `/api/hook/event` 收到对应 POST
**来源**: raw_requirements C.8; Wave 4 重构 (2026-04-27 hook bridge ADR — reference/f18-tui-bridge/)

### FR-009: 通过 settings.json hooks 桥接 PreToolUse(AskUserQuestion) 至 harness
**优先级**: Must
<!-- Wave 4: Modified 2026-04-27 — 从 stream-json 解析切换为 settings.json hooks → harness POST /api/hook/event 桥；mapper 输出 HilQuestion[] -->
<!-- Wave 4.1: Modified 2026-04-27 — unified Esc-text 协议（FR-053 默认）下，audit 闭环改为 PreToolUse + UserPromptSubmit + Stop 三 hook 联合判定（PostToolUse(AskUserQuestion) 在 unified 路径下不 fire，由设计预期）；hook 类型集合扩展为 8 类。 -->
**EARS**: While 交互 ticket 处于 running, the system shall 通过 settings.json hooks PreToolUse(AskUserQuestion) → harness `POST /api/hook/event` 接收 hook stdin JSON (含 `session_id` / `tool_use_id` / `tool_input.questions[]`)，并提取 `questions[]` 字段 (`header` / `question` / `options[]` / `multiSelect` / `allowFreeformInput`)。Wave 4.1 起 settings.json 同时注册 `Stop / SubagentStop / UserPromptSubmit / Notification` 四类 catch-all hooks，与 PreToolUse / PostToolUse / SessionStart / SessionEnd 共 8 类，支撑 unified Esc-text 协议（FR-053）的 audit 闭环。
**可视化输出**: 捕获成功时 TicketStream 对应 ticket 卡片时间线出现 "HIL" 节点。
**验收准则**:
- Given hook bridge 收到 PreToolUse(AskUserQuestion) JSON，when HookEventMapper 处理，then `HilQuestion[]` 非空且字段齐全
- Given 同 session 多 turn HIL，when 比较 `session_id`，then 跨 turn 稳定可作为 lifecycle 锚
- Given `questions[]` 缺字段，when mapper 处理，then 用默认值补齐 + warning
- **Wave 4.1 AC-2 修订**：Given 一次完整 HIL 答复（unified Esc-text 默认路径），when 统计 hook fire，then `PreToolUse(AskUserQuestion) ≥ 1` + 紧随其后 `UserPromptSubmit ≥ 1` + `Stop ≥ 1`（PostToolUse 在 unified 路径下不强制要求，设计预期不 fire）。Baseline `<N>\r` 路径仍以 `PostToolUse(AskUserQuestion) 计数 == AskUserQuestion 调用数` 验证。
**来源**: raw_requirements C.9; Wave 4 重构 (2026-04-27 hook bridge ADR); Wave 4.1 unified Esc-text 协议 (2026-04-27 增量 PoC 实测裁决)
...
### FR-011: 通过 PTY stdin 写 TUI 键序回写原交互会话
**优先级**: Must
<!-- Wave 4: Modified 2026-04-27 — 从 stream-json 文本注入切换为 TUI 键序：option=<N>\r / 自由文本 bracketed paste / 多 question 串行写键 -->
**EARS**: When Harness User 在 HILInbox 提交答案, the system shall 通过 PTY stdin 写 TUI 键序回写：option 选择写 `<N>\r`（1-based）；多 question 表单串行逐题写键并等待下一题渲染；自由文本使用 bracketed paste `\x1b[200~<text>\x1b[201~\r`。
**可视化输出**: 提交后 TicketStream 该 ticket 时间线出现 "Answered" 节点；TUI 渲染 "● User answered Claude's questions: ⎿ · ... → option_label"；ticket 同 pid 续跑。
**验收准则**:
- Given 单题 HIL with options，when 写键序 `N\r`，then claude TUI 渲染 "● User answered Claude's questions: ⎿ · ... → option_label" 且同 pid 续跑
- Given 多 question 表单，when 串行写键，then 每题渲染完成后再写下一题
- Given 自由文本含特殊字符，when bracketed paste 写入，then 子进程读取的 prompt 与原文 byte-equal
- Given pty 已关闭，when 尝试写键序，then ticket failed + HIL 答案保留
**来源**: raw_requirements C.11; Wave 4 重构 (2026-04-27 TUI 键序协议 — reference/f18-tui-bridge/puncture.py)
...
**优先级**: Must
<!-- Wave 4: NO_CHANGE 2026-04-27 — 文字保留；TDD Green 阶段在新 hook bridge 实现下重跑 20-round ≥95% PoC -->
...
**来源**: raw_requirements C.13; Wave 4 (2026-04-27) 在 hook bridge 实现下重跑相同验收

### FR-014: [DEPRECATED Wave 4] Skill 终止横幅与未答 HIL 冲突时 HIL 优先
**优先级**: ~~Must~~ → [DEPRECATED Wave 4]
<!-- Wave 4: DEPRECATED 2026-04-27 — hook bridge 模式下 SessionEnd hook 必触发；终止与未答 HIL 协调改用 hook event_name + tool_use_id queue 处理；BannerConflictArbiter 模块整体弃用 -->
**[DEPRECATED Wave 4: hook bridge 模式下 SessionEnd hook 必触发；终止与未答 HIL 协调改用 hook event_name + tool_use_id queue 处理]**
**EARS（保留原文）**: If 一张 ticket 同时出现 skill 终止横幅和未答 HIL 问题, then the system shall 优先暂停并收集 HIL 答案，在答完后重跑 phase_route 而非直接结束 ticket。
**可视化输出**: TicketStream 该卡片同时显示 "terminated banner" + "hil_waiting"，answered 后 "rerunning phase_route"。
**验收准则（原文保留 · Wave 4 起不再独立验收，由 SessionEnd hook + PreToolUse(AskUserQuestion) tool_use_id queue 自然替代）**:
...
**来源**: raw_requirements C.14; Wave 4 (2026-04-27) 弃用原因：hook bridge 模式下 SessionEnd hook 必触发，banner 文本检测路径不再需要

### D. 工具适配（Tool Adapter）

### FR-015: ToolAdapter 接口定义（Wave 4: 7 方法 Protocol）
**优先级**: Must
<!-- Wave 4: Modified 2026-04-27 — 新增 prepare_workdir 替代裸 spawn 流；map_hook_event 替代 extract_hil；签名升级到 7 方法 -->
**EARS**: The system shall 提供 `ToolAdapter` Python Protocol，方法签名: `build_argv` / `prepare_workdir` / `spawn` / `map_hook_event` / `parse_result` / `detect_anomaly` / `supports`。`prepare_workdir` 替代裸 spawn 流（确保 settings.json + .claude.json + bridge script 在 spawn 前预置）；`map_hook_event` 替代 `extract_hil`（输入 HookEventPayload，输出 `HilQuestion[]`）。
**可视化输出**: N/A — backend-only
**验收准则**:
- Given Protocol 定义，when `mypy --strict` 检查 ClaudeCodeAdapter + OpenCodeAdapter，then 符合 Protocol
- Given mock Provider 实现 7 方法，when orchestrator 调用，then FR-018 向后兼容不破坏
**来源**: raw_requirements D.15; Wave 4 重构 (2026-04-27 hook bridge ADR)
...
**可视化输出**: N/A — backend-only
**验收准则**:
- Given 一个 provider mock 实现了 Protocol 全部 6 方法（Wave 4 起为 7 方法 — FR-015），when 注册到 orchestrator，then 能被正常 dispatch
**来源**: raw_requirements D.18

### FR-051: prepare_workdir 三件套预置
**优先级**: Must
<!-- Wave 4: NEW 2026-04-27 — 实现 ESI §1.4 隔离写路径白名单 + settings.json/.claude.json 字段契约；NFR-009 强化为可测断言 -->
**EARS**: When `prepare_workdir` 被调用 for Claude ticket, the system shall 在 `<workdir>` 内预置：
- (a) `<workdir>/.claude.json`（`hasCompletedOnboarding=true`, `projects.<cwd>.hasTrustDialogAccepted=true`, `lastOnboardingVersion` 与 CLI 版本一致, `projectOnboardingSeenCount≥1`）；
- (b) `<workdir>/.claude/settings.json`（`env` 块: provider routing；`hooks` 块: PreToolUse(AskUserQuestion)/PostToolUse(*)/SessionStart/SessionEnd 全指向 `<workdir>/.claude/hooks/<bridge>`；`skipDangerousModePermissionPrompt=true`）；
- (c) `<workdir>/.claude/hooks/<bridge>` 可执行（exec bit `0o755`）。

env `HOME=<workdir>`；argv `--setting-sources project`；切断 user-scope。
**可视化输出**: N/A — backend-only（TicketStream 卡片 dispatch.workdir 字段展示隔离根路径）
**验收准则**:
- Given `prepare_workdir(spec)`，when 完成，then 三件套文件存在且 schema 校验通过
- Given run 前后 sha256 比对 `~/.claude/settings.json` + `~/.claude.json`，then 两 hash 不变（NFR-009 强化）
**来源**: Wave 4 NEW (2026-04-27 hook bridge ADR · reference/f18-tui-bridge/README.md §三件套预置)

### FR-052: HIL 应答唯一通道
**优先级**: Should
<!-- Wave 4: NEW 2026-04-27 — TUI 表单虽可见但 PTY focus 由 PtyWorker 独占；UI 文案补字与 NFR-011 协同 -->
**EARS**: While ticket 处于 `hil_waiting`, the system shall 仅接受 harness UI 经 `/api/hil/:ticket_id/answer` 提交的 HIL 答案；TUI 表单虽可见但 PTY focus 由 PtyWorker 独占；HILInbox 控件顶部添加文案 "请在此处应答（不要直接操作 TUI）"。
**可视化输出**: HILInbox 卡片顶部含 "请在此处应答（不要直接操作 TUI）" 提示文案。
**验收准则**:
- Given `hil_waiting` ticket，when 用户经 harness UI 提交答案，then PTY 写键序成功且 ticket 续跑
- Given UI 渲染，when 检查 HilControl 头部文案，then 含 "请在此处应答" 提示（NFR-011 扩展）
**来源**: Wave 4 NEW (2026-04-27 hook bridge ADR · TUI 与 HILInbox 同步显示协调)

### FR-053: TuiKeyEncoder 协议
**优先级**: Must
<!-- Wave 4: NEW 2026-04-27 — TUI 键序编码协议固化；与 ASM-010 协同；TDD Red 阶段以本协议为契约写测试 -->
<!-- Wave 4.1: Modified 2026-04-27 — 默认协议切换为 unified Esc-text（ESC + bracketed-paste(merged_text) + CR），覆盖 single-select / multi-select / multi-question / freeform 四类 HIL 答案；baseline `<N>\r` 路径降级为 prefer_baseline=True 兼容 fallback。 -->
**EARS**: When `TuiKeyEncoder` 编码 HIL 答案, the system shall 按以下协议输出 byte 序列：
- (a) **default · unified Esc-text (Wave 4.1)**: bytes `b"\x1b" + b"\x1b[200~" + merged_text.encode("utf-8") + b"\x1b[201~" + b"\r"`。`merged_text` 由 harness UI 端把所有 question 的答复（单选/多选 label / freeform 文本 / 多 question 拼接）合并为一段文本一次注入。审计闭环走 PreToolUse(AskUserQuestion) + UserPromptSubmit + Stop 三 hook（PostToolUse 在本路径下不 fire，设计预期）。
- (b) **fallback · baseline (Wave 4 原协议保留)**: 单选/多选每选项序号 N (1-based) → bytes `f"{N}\r"`；多 question 表单逐题写键 + 等待 TUI 下一题渲染（poll PTY pattern）；自由文本 bytes `"\x1b[200~" + text.encode() + "\x1b[201~\r"`。仅在 `HilWriteback(prefer_baseline=True)` 时启用，作为兼容回归路径。
- (c) **中断**: bytes `"\x03"`；**退出**: bytes `"\x03\x03"` 或 `"/exit\r"`

**可视化输出**: N/A — backend-only
**验收准则**:
- **Wave 4.1 AC-default**：Given merged_text="Python"，when `encode_unified_answer`，then bytes `== b"\x1b\x1b[200~Python\x1b[201~\r"`
- **Wave 4.1 AC-default-multi**：Given merged_text="Python, Go"（multi-select 合并），when `encode_unified_answer`，then bytes `== b"\x1b\x1b[200~Python, Go\x1b[201~\r"`
- **Wave 4.1 AC-default-utf8**：Given merged_text 含 UTF-8 多字节字符，when encode，then bytes byte-equal 守恒
- **Wave 4.1 AC-default-sec**：Given merged_text 含 `\x03 / \x04 / \x1b`，when encode，then `EscapeError`（防协议外 ESC 注入）
- Given 单题 option=1（baseline），when encode，then bytes `== b"1\r"`
- Given 自由文本 "hello"（baseline），when encode，then bytes `== b"\x1b[200~hello\x1b[201~\r"`
- Given 多 question 表单 `[Q1=opt2, Q2=opt1]`（baseline），when 顺序写键 + 等待 PTY render，then ticket 跨两题续跑
**来源**: Wave 4 NEW (2026-04-27 hook bridge ADR · reference/f18-tui-bridge/puncture.py 实测键序协议); Wave 4.1 unified Esc-text 协议 (2026-04-27 主 agent 穿刺实测三方案对比裁决：方案 A 通过 PreToolUse + UserPromptSubmit + Stop 形成 audit 闭环；claude DONE 输出 + 不重问)
...
<!-- Wave 4: Modified 2026-04-27 — IFR-001 重写：从 stream-json 切换为 PTY 双向 + workdir hooks bridge；新增 SEC 与 hook payload schema AC -->

### IFR-001 Wave 4 细化：Claude Code CLI 集成方式

Claude Code CLI 集成方式 (Wave 4 起 / Wave 4.1 修订):
- **PTY 双向 (FR-008/016)**: TUI 模式 + PTY stdin/stdout 双向通道；argv 永禁 `-p / --print / --output-format / --include-partial-messages`
- **workdir-scoped settings.json hooks (FR-009)**: 8 类 hook 注册 → harness `POST /api/hook/event`
  - PreToolUse(AskUserQuestion) / PostToolUse(*) / SessionStart / SessionEnd（Wave 4 原 4 类）
  - Stop / SubagentStop / UserPromptSubmit / Notification（**Wave 4.1 (2026-04-27) NEW** — unified Esc-text 协议 audit 闭环依赖；后 4 类为 catch-all 无 matcher）
- **Schema**:
  - argv flags 见 FR-016 严格模板
  - Hook stdin JSON 字段集: `session_id`, `transcript_path`, `cwd`, `hook_event_name`, `tool_name`, `tool_use_id?`, `tool_input?`, `ts`；`hook_event_name` 枚举为 ASM-009 锁定的 8 类
  - TUI 键序协议 (FR-053):
    - **default · unified Esc-text (Wave 4.1)**: `ESC + bracketed-paste(merged_text) + CR`；harness UI 把所有 question 答复合并文本一次注入
    - **baseline · fallback**: option=`<N>\r` / 自由文本 bracketed paste / 中断 `\x03`（保留作兼容路径，仅 `prefer_baseline=True` 启用）
- **SEC**: env 白名单不含 `ANTHROPIC_*`（routing 经 settings.json `env` 块）；隔离三件套 (.claude.json + .claude/settings.json + .claude/hooks/<bridge>) 强制 user-scope (`~/.claude/settings.json` + `~/.claude.json`) sha256 不变（NFR-009 强化 + FR-051 落地）

**验收准则（Wave 4 新增）**:
- AC-w4-1: Given run 前后 sha256 `~/.claude/settings.json` + `~/.claude.json`，when 完整 run（含 ≥1 HIL），then 两文件 hash 不变
- AC-w4-2: Given hook bridge `POST /api/hook/event` payload schema 不符，when 接收，then 415/422 拒绝且 ticket 不卡死
