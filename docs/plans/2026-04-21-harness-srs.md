...
2. **`settings.json` 字段依赖清单**（workdir-scoped，仅写入 `.harness-workdir/<run-id>/.claude/settings.json`）:
   ...
   - `hooks.PostToolUse(*)`: 工具结果回流（lifecycle 锚定）
...
**EARS**: While 交互 ticket 处于 running, the system shall 通过 settings.json hooks PreToolUse(AskUserQuestion) → harness `POST /api/hook/event` 接收 hook stdin JSON (含 `session_id` / `tool_use_id` / `tool_input.questions[]`)，并提取 `questions[]` 字段 (`header` / `question` / `options[]` / `multiSelect` / `allowFreeformInput`)。
...
**来源**: raw_requirements C.9; Wave 4 重构 (2026-04-27 hook bridge ADR)
...
- (a) **单选/多选**: 每选项序号 N (1-based) → bytes `f"{N}\r"`；多 question 表单逐题写键 + 等待 TUI 下一题渲染（poll PTY pattern）
- (b) **自由文本**: bytes `"\x1b[200~" + text.encode() + "\x1b[201~\r"` (bracketed paste + CR)
- (c) **中断**: bytes `"\x03"`；**退出**: bytes `"\x03\x03"` 或 `"/exit\r"`

**可视化输出**: N/A — backend-only
**验收准则**:
- Given 单题 option=1，when encode，then bytes `== b"1\r"`
- Given 自由文本 "hello"，when encode，then bytes `== b"\x1b[200~hello\x1b[201~\r"`
- Given 多 question 表单 `[Q1=opt2, Q2=opt1]`，when 顺序写键 + 等待 PTY render，then ticket 跨两题续跑
**来源**: Wave 4 NEW (2026-04-27 hook bridge ADR · reference/f18-tui-bridge/puncture.py 实测键序协议)
...
|----|----------------|-----------|----------|-------------|
| IFR-001 | Claude Code CLI | Outbound（spawn）+ Bidirectional（pty stdin/stdout + workdir hooks bridge）| PTY 双向 + workdir-scoped settings.json hooks → harness HTTP `POST /api/hook/event` | argv flags 见 FR-016 + Hook stdin JSON 字段集 (`session_id`, `transcript_path`, `hook_event_name`, `tool_name`, `tool_use_id?`, `tool_input?`, `ts`) + TUI 键序 (FR-053) |
...
Claude Code CLI 集成方式 (Wave 4 起):
- **PTY 双向 (FR-008/016)**: TUI 模式 + PTY stdin/stdout 双向通道；argv 永禁 `-p / --print / --output-format / --include-partial-messages`
- **workdir-scoped settings.json hooks (FR-009)**: PreToolUse(AskUserQuestion) / PostToolUse(*) / SessionStart / SessionEnd → harness `POST /api/hook/event`
- **Schema**:
  - argv flags 见 FR-016 严格模板
  - Hook stdin JSON 字段集: `session_id`, `transcript_path`, `cwd`, `hook_event_name`, `tool_name`, `tool_use_id?`, `tool_input?`, `ts`
  - TUI 键序: 见 FR-053 协议（option=`<N>\r` / 自由文本 bracketed paste / 中断 `\x03`）
...
| ASM-009 | Claude CLI hook stdin JSON schema 在 v2.1.119 起字段稳定 (`session_id`, `transcript_path`, `cwd`, `hook_event_name`, `tool_name`, `tool_use_id`, `tool_input.questions[]`)。 | CLI 升级若改 schema 字段名/嵌套结构，需触发 ESI §1.4 修订与 hook_mapper 更新（Wave 4 · 2026-04-27 新增） |
| ASM-010 | Claude TUI 键序协议在 v2.1.119 起稳定 (option menu = `"<N>\r"` / prompt = bracketed paste + CR)。 | 若 TUI 升级改菜单交互方式 (e.g., fzf-style)，需触发 ESI §1.4 修订与 TuiKeyEncoder (FR-053) 更新（Wave 4 · 2026-04-27 新增） |
