...
- 本技术栈下真实测试示例：
  - `tests/integration/test_f18_real_cli.py::test_t30_real_claude_hil_round_trip` — 真起 claude CLI v2.1.119 + hook bridge，20 次跨 user_turn HIL round-trip（FR-013 / Wave 4 TUI + Hook Bridge）
...
UI 特性（`ui: true`；本项目共 3 条：F12 / F21 / F22；Wave 2 后 F13/F14→F21、F15/F16→F22）在 ST 阶段必须用 Chrome DevTools MCP 做**正面视觉存在**断言：
...
| 8 | 依赖未 `passing` 的 feature 不得开工；违反时 router 会 refuse，强跑会破坏依赖链（见 Design §11.3） | 循环阻塞；HIL PoC 未过就跑 F20/F21 会浪费会话 |
